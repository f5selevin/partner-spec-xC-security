...
1. In **Web App & API Protection**, go to **Load Balancers** > **Origin Pools**.
2. Create the following four origin pools:
   ...
   .. table:: Required origin pools
      ...
      =================================  =============================================
      ``openbanking-banks``              ``openbanking-banks.$$namespace$$``
      ``openbanking-accounts``           ``openbanking-accounts.$$namespace$$``
      ``openbanking-payments``           ``openbanking-payments.$$namespace$$``
      ``openbanking-swagger``            ``openbanking-swagger.$$namespace$$``
      =================================  =============================================
...
3. Configure each origin pool with port ``80`` and add a **K8s Service Name of
   ...
   with ``arcadia-openbanking-vk8s``, and select **vK8s Networks on Site**.
4. Create the HTTP load balancer ``arcadia-openbanking`` with the domain
   ``$$namespace$$.spec-security.f5se.com``. Use automatic certificate
   management.
5. Add these routes to the HTTP load balancer:
   ...
   .. table:: Required HTTP routes
      ...
      ====================  ============================
      ``/banks``            ``openbanking-banks``
      ``/accounts``         ``openbanking-accounts``
      ``/payments``         ``openbanking-payments``
      ``/swagger``          ``openbanking-swagger``
      ====================  ============================
...
9. Open the interactive Swagger UI at
   :ext_link:`https://$$namespace$$.spec-security.f5se.com/swagger/`. Expand an
